#!/usr/bin/env bash
# check-cortex-version.sh — luna-lite preinstall gate
#
...
#   2. cortex >= 6.9.0
#
# Luna-Lite is SURFACE-NEUTRAL, so — unlike pier — this gate requires NO
...
#
# arc install luna-lite runs this BEFORE dropping any files. On failure the
...
if ! command -v cortex &>/dev/null; then
  echo "luna-lite preinstall ERROR: 'cortex' binary not found on PATH." >&2
...
if [[ -z "$CORTEX_VERSION" ]]; then
  echo "luna-lite preinstall ERROR: could not determine cortex version." >&2
  exit 1
fi
...
if (( MAJOR < REQUIRED_MAJOR || (MAJOR == REQUIRED_MAJOR && MINOR < REQUIRED_MINOR) )); then
  echo "luna-lite preinstall ERROR: cortex ${CORTEX_VERSION} found; >= ${REQUIRED_MAJOR}.${REQUIRED_MINOR}.0 required." >&2
  echo "  Upgrade:  arc upgrade cortex" >&2
  exit 1
fi

echo "luna-lite preinstall: cortex ${CORTEX_VERSION} OK — installing the Luna fragment."
