#!/usr/bin/env bash
# signal-cortex-reload.sh — luna-light postinstall (step 1 of 1)
#
...
# THEN runs this script. Luna-Light is an in-process agent sharing the stack's
...
if command -v cortex &>/dev/null; then
  echo "luna-light postinstall: signalling cortex agents reload..."
  if timeout "$RELOAD_TIMEOUT" cortex agents reload 2>/dev/null; then
    echo "luna-light postinstall: reload acknowledged — Luna is active on your stack."
    exit 0
  fi
  echo "luna-light postinstall: 'cortex agents reload' timed out or failed; trying SIGHUP..." >&2
fi
...
if [[ -f "$PID_FILE" ]]; then
  ...
  if kill -0 "$DAEMON_PID" 2>/dev/null; then
    echo "luna-light postinstall: sending SIGHUP to cortex daemon (pid ${DAEMON_PID})..."
    kill -HUP "$DAEMON_PID"
    echo "luna-light postinstall: SIGHUP sent — daemon reloads agents.d/ on next poll."
    exit 0
  fi
fi

# ── 3. No running daemon — fragment is on disk, active on next start ──────
echo "luna-light postinstall: cortex daemon not detected." >&2
